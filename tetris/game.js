(() => {
  ...
  const COLORS=[null,'#52e3ff','#4f79ff','#ff9d3d','#ffe363','#6ee976','#c674ff','#ff4fa3'];
  ...
  function draw(){ctx.fillStyle='#090b18';ctx.fillRect(0,0,COLS,ROWS);ctx.strokeStyle='#171a2d';ctx.lineWidth=.025;for(let x=0;x<=COLS;x++){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,ROWS);ctx.stroke()}for(let y=0;y<=ROWS;y++){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(COLS,y);ctx.stroke()}drawMatrix(arena,{x:0,y:0});if(player)drawMatrix(player.matrix,player.pos)}
  function drawNext(){nextCtx.fillStyle='#090b18';nextCtx.fillRect(0,0,5,5);const width=next[0].length,height=next.length;drawMatrix(next,{x:(5-width)/2,y:(5-height)/2},nextCtx)}
