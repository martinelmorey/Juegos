(() => {
  ...
  const COLORS=[null,'#0f8f38','#fff','#0f8f38','#fff','#0f8f38','#fff','#0f8f38'];
  ...
  let arena, player, next, score, lines, level, dropCounter, dropInterval, lastTime, running=false, paused=false, impact=0, flash=0, particles=[];
  ...
  function burst(x,y,count=3){for(let i=0;i<count;i++)particles.push({x:x+.5,y:y+.5,vx:(Math.random()-.5)*.15,vy:-Math.random()*.16,life:1,size:.06+Math.random()*.1})}function sweep(){let rowCount=1,cleared=0;outer:for(let y=arena.length-1;y>=0;y--){for(let x=0;x<COLS;x++)if(arena[y][x]===0)continue outer;for(let x=0;x<COLS;x++)burst(x,y,2);arena.splice(y,1);arena.unshift(Array(COLS).fill(0));y++;cleared++;score+=rowCount*100*level;rowCount*=2}if(cleared){lines+=cleared;level=Math.floor(lines/10)+1;dropInterval=Math.max(120,800-(level-1)*65);flash=10+cleared*3;impact=8+cleared*2;updateStats()}}
  function rotate(matrix,dir){const rotated=matrix[0].map((_,x)=>matrix.map(row=>row[x]));if(dir>0)rotated.forEach(row=>row.reverse());else rotated.reverse();matrix.splice(0,matrix.length,...rotated)}
  ...
  function hardDrop(){if(!running||paused)return;impact=5;let distance=0;while(!collide(arena,player)){player.pos.y++;distance++}player.pos.y--;score+=(distance-1)*2;merge(arena,player);sweep();spawn();updateStats();dropCounter=0}
  ...
  function draw(){const shake=impact>0?(Math.random()-.5)*impact*.022:0;ctx.save();ctx.translate(shake,shake);ctx.fillStyle=flash>0?'#0f8f38':'#000';ctx.fillRect(0,0,COLS,ROWS);ctx.strokeStyle='#1a1a1a';ctx.lineWidth=.025;for(let x=0;x<=COLS;x++){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,ROWS);ctx.stroke()}for(let y=0;y<=ROWS;y++){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(COLS,y);ctx.stroke()}drawMatrix(arena,{x:0,y:0});if(player){const ghost={matrix:player.matrix,pos:{x:player.pos.x,y:player.pos.y}};while(!collide(arena,ghost))ghost.pos.y++;ghost.pos.y--;ctx.globalAlpha=.22;drawMatrix(ghost.matrix,ghost.pos);ctx.globalAlpha=1;drawMatrix(player.matrix,player.pos)}particles.forEach(p=>{ctx.globalAlpha=Math.max(0,p.life);ctx.fillStyle='#fff';ctx.fillRect(p.x,p.y,p.size,p.size)});ctx.globalAlpha=1;ctx.restore()}
  ...
  function updateStats(){scoreEl.textContent=score;linesEl.textContent=lines;levelEl.textContent=level;if(scoreEl.animate)scoreEl.animate([{transform:'scale(1)'},{transform:'scale(1.22)'},{transform:'scale(1)'}],{duration:240,easing:'ease-out'})}
  function update(time=0){const delta=time-lastTime;lastTime=time;if(running&&!paused){dropCounter+=delta;impact=Math.max(0,impact-delta*.025);flash=Math.max(0,flash-delta*.03);particles=particles.filter(p=>{p.x+=p.vx;p.y+=p.vy;p.vy+=.008;p.life-=delta*.0018;return p.life>0});if(dropCounter>dropInterval)drop();draw();requestAnimationFrame(update)}}
  ...
  document.addEventListener('keydown',e=>{const keys=['ArrowLeft','ArrowRight','ArrowDown','ArrowUp',' '];if(keys.includes(e.key))e.preventDefault();if(e.key==='ArrowLeft')move(-1);if(e.key==='ArrowRight')move(1);if(e.key==='ArrowDown')drop();if(e.key==='ArrowUp')playerRotate();if(e.key===' ')hardDrop();if(e.key==='p'||e.key==='P')togglePause();if(e.key==='r'||e.key==='R')start()});
